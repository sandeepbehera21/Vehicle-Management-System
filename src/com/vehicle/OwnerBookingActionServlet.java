    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if ("approve".equalsIgnoreCase(action)) {
                ...
                // Vehicle goes On Trip and not available
                try (PreparedStatement ps = con.prepareStatement("UPDATE vehicle SET availability=false, status='On Trip' WHERE vehicle_id=?")) {
                ...
                // Free the vehicle
                try (PreparedStatement ps = con.prepareStatement("UPDATE vehicle SET availability=true, status='Available' WHERE vehicle_id=?")) {
                    ps.setInt(1, vehicleId);
                    ps.executeUpdate();
                }
                ...
                // Available after completing the trip
                try (PreparedStatement ps = con.prepareStatement("UPDATE vehicle SET availability=true, status='Available' WHERE vehicle_id=?")) {
                    ps.setInt(1, vehicleId);
                    ps.executeUpdate();
                }
